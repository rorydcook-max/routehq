-- 0071: let the server countersign rental agreements automatically.
--
-- Customers complete bookings online and sign in the same visit. For that,
-- the business's side of the agreement must be finalised without an operator
-- present, using the signature the business pre-authorised in its settings
-- (the signature row is already recorded as 'stored_business_authorisation',
-- with the accepted authorisation-text version).
--
-- create_rental_document_draft_version (last defined in 0056_transactional_rental_document_draft_versions.sql) and
-- finalise_rental_document_version (last defined in 0064) now accept calls
-- from the service role - held only by the RouteHQ server, never a browser -
-- in addition to signed-in operators. Only the two caller checks change:
--   * 'Authentication is required' now also passes for the service role
--   * the operator-role check applies to signed-in users only
-- Every other check is unchanged: document/organisation match, content hash,
-- final PDF path rules, and the signatory, signature and consent version
-- having to match the immutable business snapshot.
...
begin
  -- The RouteHQ server (service role) may finalise under the business's stored
  -- signature authorisation, e.g. when a customer completes their booking online.
  if v_actor_id is null and coalesce(auth.role(), '') <> 'service_role' then
  ...
  if v_actor_id is not null and not public.has_org_role(v_document.organization_id, array['owner','manager','operator']::public.organization_role[]) then
...
declare
  ...
  v_version public.rental_document_versions%rowtype;
begin
  -- The RouteHQ server (service role) may create drafts, e.g. when a customer completes a booking.
  if v_actor_id is null and coalesce(auth.role(), '') <> 'service_role' then
    raise exception 'Authentication is required.';
  end if;

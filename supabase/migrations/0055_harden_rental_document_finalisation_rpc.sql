-- Harden rental document finalisation so direct RPC calls cannot bypass storage,
-- feature, signature, or snapshot eligibility checks.
...
begin
  ...
  if length(p_final_pdf_storage_path) > 512
    ...
    or lower(p_final_pdf_storage_path) like '%2e%'
    or lower(p_final_pdf_storage_path) like '%2f%'
    or lower(p_final_pdf_storage_path) like '%5c%'
  then
  ...
  if p_signature_storage_bucket <> 'branding'
    ...
    or p_signature_storage_path like '%//%'
    or p_signature_storage_path like '%://%'
    or lower(p_signature_storage_path) like '%2e%'
    or lower(p_signature_storage_path) like '%2f%'
  then

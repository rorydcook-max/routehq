-- 0064: remove the last organisation feature flag from finalisation.
--
-- 0063 removed the contract authority mode and the customer-signing flag
-- checks, but finalise_rental_document_version (last defined in 0058_fix_finalisation_rpc_encoded_path_validation.sql)
-- still refused to run unless organizations.settings carried
-- rental_document_engine_enabled or experimental_rental_document_engine_enabled.
-- With a single authority path nothing sets those settings any more, so every
-- finalisation would fail. The function is recreated verbatim minus that check.
-- Role, organisation, hash and path checks are unchanged.
